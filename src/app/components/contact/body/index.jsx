        ...
        try {
            const json = await fetch('https://76h86gyo24.execute-api.us-west-2.amazonaws.com/prod/contactUs', {
                ...
                body: JSON.stringify({
                    ...
                    message: { firstName, lastName, phoneNumber, email, description: message, role, companyName },
            ...
            <form action="" className="sm:mt-[7rem] mt-[3rem]">
                <div className="sm:flex justify-between gap-[2rem]">
                    <div className="flex py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem] sm:flex-1">
                        ...
                        <input type="text" required value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full text-black focus:outline-none text-[1.2rem]" placeholder="First Name*" />
                    </div>
                    <div className="flex py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem] sm:flex-1">
                        <img src="/name_.svg" className="w-[1.6rem]" alt="name" />
                        <input type="text" required value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full text-black focus:outline-none text-[1.2rem]" placeholder="Last Name*" />
                    </div>
                </div>
                <div className="sm:flex justify-between gap-[2rem]">
                    <div className="flex py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem] sm:flex-1">
                        <img src="/email.svg" className="w-[1.6rem]" alt="name" />
                        <input type="text" value={email} onChange={(e) => setEmail(e.target.value)} className="sm:w-full w-auto text-black focus:outline-none text-[1.2rem]" placeholder="Email Address" />
                    </div>
                    <div className="flex py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem] sm:flex-1">
                        <PhoneInput country={'us'} value={phoneNumber} onChange={(value) => setPhoneNumber(value)} type="tel" className="sm:w-full phone-input w-auto text-black focus:outline-none text-[1.2rem]" placeholder="Phone Number" />
                    </div>
                </div>
                <div className="sm:flex justify-between gap-[2rem]">
                    <div className="flex py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem] sm:flex-1">
                        <img src="/company.svg" className="w-[1.6rem]" alt="name" />
                        <input type="text" value={companyName} required onChange={(e) => setCompanyName(e.target.value)} className="sm:w-full text-black focus:outline-none text-[1.2rem]" placeholder="company*" />
                    </div>
                    <div className="flex py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem] sm:flex-1">
                        <img src="/role.svg" className="w-[1.6rem]" alt="name" />
                        <input type="text" value={role} onChange={(e) => setRole(e.target.value)} className="sm:w-full text-black focus:outline-none text-[1.2rem]" placeholder="Role" />
                    </div>
                </div>
                <div className="flex items-start  py-[.8rem] gap-x-[1rem] border-b border-b-black mb-[3rem]">
                    ...
                    <textarea type="text" value={message} onChange={(e) => setMessage(e.target.value)} className="sm:w-full text-black focus:outline-none text-[1.2rem] h-[8rem] sm:h-[14rem] resize-none" placeholder="Project Description" />
                </div>
