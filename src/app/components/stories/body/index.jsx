...
const Index = () => {
    ...
    return (
        <section className="responsive-grid px-[3rem] sm:px-[10rem] sm:py-[10rem] py-[5rem] bg-white text-black">
            {cards.map((card, index) => <Card card={card} key={index} id={index} />)}
        </section>
    );
}
...
const Card = ({ card, id }) => {
    return (
        <div className="w-[35rem]">
            ...
            <Link  href={`/Stories/${id}`} className="sm:mt-[3.5rem] mt-[2rem] text-[1.4rem] font-[700] border sm:px-[2.5rem] rounded-[1rem] inline-block  sm:py-[1.2rem] px-[2rem] py-[1rem] border-[#232323] hover:scale-[1.1] transition-all">Learn More</Link>
        </div>
    )
}