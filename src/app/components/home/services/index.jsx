      ...
      <div className=" px-[3rem] sm:px-[10rem] mt-[6rem] ">
        ...
        <div className="pl-[8rem] pr-[4rem] py-[7rem] hidden bg-black sm:flex justify-between w-[100%] mt-[4rem]">
          <div className="w-[450px]">
            ...
            <h3 className="font-[600] mb-1 text-[2rem] sm:mb-[2rem] text-center sm:text-start">
              DevSecOps
            </h3>
            <p className="text-[1.2rem] sm:text-[1.7rem] text-center sm:text-start">
              Bridging the gap between development, security, and operations.
              Achieve efficient and secure software delivery through
              collaboration, automation, and a security-first approach.
            </p>
          </div>
          <Image
            width={657}
...
const ServiceDesktop = ({ services }) => {
  ...
  const toggleActive = (id) => {
    setServices((prevServices) =>
      prevServices.map((item, i) =>
        id == i ? { ...item, active: true } : { ...item, active: false }
      )
    );
  };
  return (
      ...
      <div className="flex justify-between w-[100%]">
        <div className="flex flex-col">
          ...
          <div className="w-[40rem]">
            <h2 className="font-[700] text-[5rem] mb-[2rem]">
              What We Can Do <span className="font-[400]">For You:</span>
            </h2>
            <p className="">
              Ace8 offers a suite of cutting-edge services, from Message
              Brokersto DevSecOps, empowering enterprises with services to
              thrive in today's digital economy. With streamlined data flow and
              robust development practices, Ace8 ensures unparalleled efficiency
              and resilience, driving extreme value for modern businesses.
            </p>
          </div>
        </div>
            ...
            <ul className="mt-[1.5rem] flex flex-col gap-[1.5rem] sm:gap-[.5rem]">
              {_services.map((service, index) => (
                <li
                  ...
                  className={`flex px-[2.8rem] py-[.8rem] hover:bg-accent-300  hover:cursor-pointer rounded-[.5rem] gap-[1rem] ${
                    service.active ? "bg-accent-300" : ""
                  }`}
                  key={index}
                >
                  <Image
          ...
          <div className="bg-[#0D1117] p-[1.5rem] rounded-[1rem]">
            {_services.map(
              (item, i) =>
                item.active && (
                  <div
                    className="bg-[#161B22] pb-[1.5rem] px-[3rem] h-[100%] rounded-[.8rem]"
                    key={item.Name}
                  >
                    {item.default ? (
                      <div className="pt-[3rem] w-[27rem] flex flex-col">
                        ...
                        <img
                          src={item.defualtImage}
                          className="w-[20rem] self-center mt-[8rem]"
                          alt="featured_icon"
                        />
                      </div>
                    ) : (
                      <React.Fragment>
                        <div className="mb-[3rem] pt-[3rem]">
                          <Image
                            ...
                            height={84}
                          />
                          <h2 className="font-[700] text-[2.5rem] w-[20rem]">
                            {item.Name}
                          </h2>
                        </div>
...
const Card = ({ service }) => (
  <Link href={service.link} className="hover:scale-[1.07] transition-all">
    <div className="bg-black py-[1.8rem] sm:py-[3rem] px-[1.2rem] sm:px-[1.6rem] w-[100%] sm:w-[38rem] flex flex-col items-center sm:items-start">
      ...
      <h4 className="font-[600] mb-1 text-[2rem] sm:mb-[2rem] text-center sm:text-start">
        {service.Name}
      </h4>
      <p className="text-[1.2rem] sm:text-[1.7rem] text-center sm:text-start">
        {service.role}
